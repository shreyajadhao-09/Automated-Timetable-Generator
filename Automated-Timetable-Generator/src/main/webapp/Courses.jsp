...
<select required name="staffName" class="form-control"  >
<option value=""><---Select Staff---></option> 
<%List<Staff> lststaff = new ArrayList<Staff>();
Staff staff=new Staff();
lststaff=staff.getStaffList1(request.getAttribute("branch").toString().trim());
for(int i=0;i<lststaff.size();i++)
{%>
	<option value="<%=lststaff.get(i).getUserid() %>"><%=lststaff.get(i).getUsernm() %></option>
<%}
 %></select>
</body>
</html>
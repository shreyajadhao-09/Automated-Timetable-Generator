...
JavaFuns jf=new JavaFuns();
 Vector v1=jf.getValue("select subName from subjects where subName<>'"+request.getParameter("sub").trim()+"' and subName<>'"+session.getAttribute("pref2").toString().trim()+"' and semester="+session.getAttribute("sem").toString().trim()+" and branch='"+session.getAttribute("branch").toString().trim()+"' and course='"+session.getAttribute("course").toString().trim()+"'", 1);
 session.setAttribute("pref3", request.getParameter("sub").trim());
 	 
 int intake=Integer.parseInt(session.getAttribute("intake").toString().trim());
 
 %> 
		
<select name="pref3" required class="form-control">
